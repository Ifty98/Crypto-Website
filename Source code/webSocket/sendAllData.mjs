...
export const handler = async (event) => {
  ...
  try {
    const connId = await getLastConnectionId();
...
// Get the most recent connected client id
async function getLastConnectionId() {
    try {
        const params = {
        ...
        if (response.Items && response.Items.length > 0) {
            // Extract the first connection ID
            const firstConnectionId = response.Items[0].ConnectionId.S;
        ...
        console.error('Error fetching last connection ID:', error);
        throw error;
    }
}
